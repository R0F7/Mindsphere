export default function CollabMarquee() {
  ...
  return (
    <section className="bg-white border-t border-b border-border py-16 overflow-hidden">
      ...
      <div className="relative overflow-hidden [mask-image:linear-gradient(90deg,transparent,black_10%,black_90%,transparent)] [-webkit-mask-image:linear-gradient(90deg,transparent,black_10%,black_90%,transparent)]">
        <div className="flex items-center w-max animate-[marquee_22s_linear_infinite]">
          {[...partners, ...partners].map((partner, idx) => (
            <span
              key={idx}
              className="flex items-center gap-3 px-9 font-['Fraunces',serif] text-[1.1rem] font-light italic text-muted whitespace-nowrap transition-colors duration-200 hover:text-primary"
            >
              {partner}
