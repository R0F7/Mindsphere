...
export default function ServiceCard({ i, item }: any) {
  return (
    <div
      data-aos="fade-up"
      data-aos-delay={i * 100}
      className={`group relative bg-white border border-border rounded-xl p-[36px_32px] 
                         transition-all duration-600 ease-in-out hover:shadow-sh3
                         `}
    >
      {/* Giant Faded Number */}
      ...
      <Link
        href={item.link}
        className="inline-flex items-center gap-2.5 text-[0.82rem] font-medium text-t2 transition-colors hover:text-text"
      >
        Learn more
        <span
          className="w-7 h-7 rounded-full border border-bd2 flex items-center justify-center text-[0.7rem] text-t3 transition-all duration-200 
                               group-hover:bg-navy group-hover:text-white group-hover:border-navy"
        >
          ↗
        </span>
      </Link>
